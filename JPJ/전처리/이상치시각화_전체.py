...
# 결과 폴더 생성
result_folder = r"D:\my_projects\natural-science-time-series\JPJ\전처리\시각화_결과"
os.makedirs(result_folder, exist_ok=True)
print(f"결과 폴더 생성: {result_folder}")

# 추가 변수 리스트 (기존에 확인하지 않은 변수들)
additional_variables = ['sisnthick', 'vxo', 'vyo', '10u', '10v', '2t', 'sdlwrf', 'sdswrf', 'snswrf', 'tprate', 'sic', 'tp']

# 1. 시계열 플롯 생성
plt.figure(figsize=(15, 30))
for i, var in enumerate(additional_variables, 1):
    plt.subplot(len(additional_variables), 1, i)
    plt.plot(df.index, df[var])
    plt.title(f'{var} 시계열 (1991-2023)')
    plt.grid(True)
plt.tight_layout()
plt.savefig(os.path.join(result_folder, 'additional_time_series_plots.png'))
plt.close()
...
# 주요 변수에 대해 계절별 분포 시각화
for var in ['10u', '10v', 'tprate']:  # 바람 및 강수 관련 변수 선택
    plt.figure(figsize=(15, 8))
...
with open(os.path.join(result_folder, 'analysis_summary.txt'), 'w', encoding='utf-8') as f:
    ...
    f.write(f"분석 데이터: all_data_month_199101_202312.csv\n")
    f.write(f"분석 변수: {', '.join(additional_variables)}\n\n")
